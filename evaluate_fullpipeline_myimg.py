...
parser.add_argument('--batch', type=int, default=1, #4, #1, #2,
...
def main():
    ...
    #myfiles = glob.glob('/l/users/20020067/Study/PhD/2 semester/CV-802 (3D vision)/Project/_Ours/PanoFusion/OmniFusion/results/stanford/512x1024/resnet34/visualize_point_2_iter/test_equi_rgb_0.png')
    myfiles = glob.glob('/l/users/20020067/Study/PhD/2 semester/CV-802 (3D vision)/Project/_Ours/PanoFusion/visualize_stanford2d3d_ours_48_24/evaluation/panofusion/my/_samples/*.jpg')
    ...
    for filename in myfiles:
        ...
        rgb = torch.from_numpy(img).float().cuda()
        render = []
        ...
        plot.imsave('{}/my/{}_coarse_vis.png'.format(
            result_view_dir, filename.split('/')[-1]), depth_coarse_img, cmap='jet')
        ...
        with torch.no_grad():
            ...
            outputs =stereo_network(render, rgb)
        ...
        print("BEFORE: Min depth:", depth_pred_img.min(), "Max depth:", depth_pred_img.max())
        #depth_prediction[depth_prediction>8] = 0
        depth_pred_img[depth_pred_img < 0.1] = 0.1
        depth_pred_img[depth_pred_img > 8] = 8
        print("AFTER: Min depth:", depth_pred_img.min(), "Max depth:", depth_pred_img.max())
        ...
        cv2.imwrite('{}/my/{}_rgb.png'.format(result_view_dir, filename.split('/')[-1]), img_np*255)
        ##for num_render in range(len(render_np)):
        render_img = render_np[0, :, :, :].transpose(1,2,0)
        cv2.imwrite('{}/my/{}_render.png'.format(result_view_dir, filename.split('/')[-1]), render_img*255)
        plot.imsave('{}/my/{}_depth_pred.png'.format(result_view_dir, filename.split('/')[-1]), depth_pred_img, cmap="jet")
