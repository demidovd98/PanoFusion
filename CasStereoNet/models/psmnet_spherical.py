...
class CostAggregation(nn.Module):
    ...
    def forward(self, cost, FineD, FineH, FineW, disp_range_samples):
        ...
        #pred0 = S360.derivatives.dispairty_to_depth_theta(sgrid, pred0-256, baseline=0.24)
        #pred1 = S360.derivatives.dispairty_to_depth_theta(sgrid, pred1-256, baseline=0.24)
        #pred2 = S360.derivatives.dispairty_to_depth_theta(sgrid, pred2-256, baseline=0.24)
        #pred3 = S360.derivatives.dispairty_to_depth_theta(sgrid, pred3-256, baseline=0.24)
        if self.training:
...
class GetCostVolume(nn.Module):
    ...
    def forward(self, x, y, disp_range_samples, ndisp):
        ...
        disp_range_samples = disp_range_samples.unsqueeze(-1) #(B, D, H, W, 1)
        disp = torch.cat(
                (
                    torch.zeros_like(disp_range_samples),
                    disp_range_samples
                ),
                dim=-1
            )  #(B, D, H, W, 2)
        ...
        #cost[:, x.size()[1]:, :, :, :] = F.grid_sample(y, grid.view(bs, ndisp * height, width, 2), mode='bilinear', padding_mode='zeros')
