...
class feature_extraction(nn.Module):
    def __init__(self, arch_mode="fpn", num_stage=None):
        ...
        self.branch4 = nn.Sequential(nn.AvgPool2d((8, 8), stride=(8,8)),
                                     ...
                                     nn.ReLU(inplace=True))
        '''
        self.aspp1 = nn.Sequential(convbn(128, 32, 1, 1, 0, 1),
                    nn.ReLU(inplace=True))
        self.aspp2 = nn.Sequential(convbn(128, 32, 3, 1, 1, 6),
                    nn.ReLU(inplace=True))
        self.aspp3 = nn.Sequential(convbn(128, 32, 3, 1, 1, 12),
                    nn.ReLU(inplace=True))
        self.aspp4 = nn.Sequential(convbn(128, 32, 3, 1, 1, 18),
                    nn.ReLU(inplace=True))
        self.aspp5 = nn.Sequential(convbn(128, 32, 3, 1, 1, 24),
                    nn.ReLU(inplace=True))
        #TODO: add inner0 to reduce chs to 32, and memory do not change
        self.inner0 = nn.Sequential(convbn(224, 128, 3, 1, 1, 1),
                                    nn.ReLU(inplace=True),
...
class CostAggregation(nn.Module):
    ...
    def forward(self, cost, FineD, FineH, FineW, depth_range_samples, interval, min_depth, max_depth):
...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right, min_depth=0.3, max_depth=8.0):
        ...
        for stage_idx in range(self.num_stage):
            ...
            if self.training:
                pred0, pred1, pred2, pred3 = self.cost_agg[stage_idx](cost,
                                                                      ...
                                                                      interval=interval,
                                                                      min_depth=cur_depth_min,
                                                                      max_depth=cur_depth_max)
                pred = pred3
                outputs_stage = {
                    "pred0": pred0,
                    "pred1": pred1,
